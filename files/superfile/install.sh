...
file_to_storage=${script_path}/packages/${file_name}.tar.gz

if [ -e "${file_to_storage}" ]; then
    # use local package
    echo -e "${bright_yellow}Using local file ${cyan}${file_to_storage}...${nc}"
    cp ${file_to_storage} "${file_name}.tar.gz"
else
